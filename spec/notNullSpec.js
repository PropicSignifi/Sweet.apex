const fs = require('fs');
const path = require('path');
const transpile = require('../src/transpiler');

const targetFile = 'NotNull';
const config = {
    features: ['not_null'],
};

function joinPath(items) {
    return items.join(path.sep);
}

describe("not_null feature", function() {
    it("should convert @notNull", function() {
        const content = fs.readFileSync(joinPath(['resources', 'not_null', targetFile + '.apex']), 'utf8');
        const result = transpile(content, config);
        const benchmark = fs.readFileSync(joinPath(['resources', 'benchmark', targetFile + '.cls']), 'utf8');

        expect(result).toEqual(benchmark);
    });
});
